...
export default function App() {
  ...
  if (useRemote && !remoteReady) {
    return (
      <div className="flex min-h-dvh flex-col items-center justify-center gap-2 bg-zinc-50 px-6 text-center dark:bg-zinc-950">
        <p className="text-sm text-zinc-600 dark:text-zinc-400">서버에서 작업을 불러오는 중…</p>
      </div>
    )
  }
  ...
  return (
    <div className="flex min-h-dvh bg-zinc-50 dark:bg-zinc-950">
      {remoteError && (
        <div className="fixed bottom-4 left-1/2 z-50 max-w-md -translate-x-1/2 rounded-lg border border-amber-200 bg-amber-50 px-4 py-2 text-center text-xs text-amber-900 shadow-sm dark:border-amber-900/80 dark:bg-amber-950/90 dark:text-amber-200">
