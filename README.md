...
- **저장:** 원격 미설정 시 브라우저 `localStorage` (`369stage-tasks`) · 원격 설정 시 Cloudflare Worker API (`GET/PUT /tasks`). 원격 사용 시 **약 30초마다 서버를 다시 읽고**, **다른 탭/앱에 갔다가 돌아오면** 즉시 한 번 더 받아 여러 PC 간 변경이 화면에 반영되기 쉽다(실시간 푸시는 아님).
  ...
  "tasks": [
    {
      ...
      "dueDate": "2026-04-15",
      "directionNotes": {
        "30": [{ "id": "…", "text": "", "createdAt": "…", "updatedAt": "…" }],
        "60": [],
        "90": []
      },
      "status": "active",
...
| `directionNotes` | 객체 | ✓ | 키 `"30"`, `"60"`, `"90"` → 각각 **노트 배열** (`id`, `text`, `createdAt`, `updatedAt` ISO 문자열) |
